...
public class LogContext : DbContext
{
    ...
    {
        optionsBuilder.UseSqlServer("Server=PC03LAB2524\\SENAI; Database=WebScrapingDb2; User Id=sa; Password=senai.123;"); // Substitua "YourConnectionString" pela sua string de conexão
    }
}
...
class Program
{
                    ...
                    foreach (Produto produto in novosProdutos)
                    {
                        if (!produtosVerificados.Exists(p => p.Id == produto.Id))
                        {
                            ...
                            if (!ProdutoJaRegistrado(produto.Id))
                            {
                                ...
                                //Pega as sttring de preço e link
                                ...
                                try
                                {
                                    ...
                                    try
                                    {
                                        //Mandar registrar o Log
                                        //Temporariamente desativado
                                        SendEmail.EnviarEmail("Produto Mercado Livre", precoObtidoMercadoLivre, linkProdutoMercadoLivre,"Produto Magazine Luiza", precoObtidoMagazineLuiza, produto.Nome, melhorCompraEmail);
    ...
    //Comparar preços
    //Erro mercado livre retorna string nula
    public static string CompararPrecos(string precoTextoMercadoLivreTexto, string linkMercadoLivre,string precoTextoMagazineLuiza, string linkMagazineLuiza, string nomeProduto)
    {
        ...
        {
            //precoTextoMercadoLivreTexto = "99999999999999999";
            Console.WriteLine("string de texto nula");
        }
        ...
        if (valorMercadoLivre < valorMagazineLuiza)
        {
            ...
            return ($"O Mercado Livre tem {nomeProduto} de menor preço: {linkMercadoLivre}");
        }
        else if (valorMercadoLivre > valorMagazineLuiza)
        {
            ...
            return ($"A Magazine Luiza tem {nomeProduto} de menor preço: {linkMagazineLuiza}");
        }
        else if (valorMercadoLivre == valorMagazineLuiza)
        {
